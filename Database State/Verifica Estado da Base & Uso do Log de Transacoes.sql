SELECT db.[name] DatabaseName
	...
	,CASE 
		...
		WHEN db.[state] = 7
			THEN 'COPYING'
		WHEN db.[state] = 10
			THEN 'OFFLINE_SECONDARY'
		END CurrentStateDB
	,db.recovery_model_desc RecoveryModel
	,db.log_reuse_wait_desc
	,CASE 
		WHEN hdr.database_state = 0
			THEN 'ONLINE'
		WHEN hdr.database_state = 1
			THEN 'RESTORING'
		WHEN hdr.database_state = 2
			THEN 'RECOVERING'
		WHEN hdr.database_state = 3
			THEN 'RECOVERY_PENDING'
		WHEN hdr.database_state = 4
			THEN 'SUSPECT'
		WHEN hdr.database_state = 5
			THEN 'EMERGENCY'
		WHEN hdr.database_state = 6
			THEN 'OFFLINE'
